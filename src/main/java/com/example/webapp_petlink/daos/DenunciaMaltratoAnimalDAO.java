...
public class DenunciaMaltratoAnimalDAO extends DaoBase {
    ...
    public boolean insertarDenuncia(DenunciaMaltratoAnimal denuncia) {
        ...
        try (Connection conn = getConnection();
            ...
            pstmt.setTimestamp(9, new Timestamp(denuncia.getFechaHoraRegistro().getTime()));
    ...
    public List<DenunciaMaltratoAnimal> obtenerTodasLasDenuncias() {
        ...
        try (Connection conn = getConnection();
            ...
            while (rs.next()) {
                denuncias.add(fetchDenunciaData(rs));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    public boolean actualizarDenuncia(DenunciaMaltratoAnimal denuncia) {
        String sql = "UPDATE DenunciaMaltratoAnimal SET nombre_foto_animal = ?, foto_animal = ?, tamanio = ?, raza = ?, descripcion_maltrato = ?, nombre_maltratador = ?, direccion_maltrato = ?, es_denuncia_activa = ?, fecha_hora_registro = ?, id_usuario_final = ?, id_estado = ?, tiene_denuncia_policial = ? " +
        ...
        try (Connection conn = getConnection();
            ...
            pstmt.setInt(11, denuncia.getEstado().getId_estado());
            pstmt.setBoolean(12, denuncia.isTieneDenunciaPolicial());
            pstmt.setInt(13, denuncia.getIdDenunciaMaltratoAnimal());
    ...
    public boolean eliminarDenuncia(int id) {
        String sql = "DELETE FROM DenunciaMaltratoAnimal WHERE id_denuncia_maltrato_animal = ?";
    ...
    private DenunciaMaltratoAnimal fetchDenunciaData(ResultSet rs) throws SQLException {
        ...
        denuncia.setFechaHoraRegistro(rs.getTimestamp("fecha_hora_registro"));
